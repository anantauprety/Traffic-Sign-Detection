...
for line in csv:
	...
	if args.mode == 'crop':
		baseName = os.path.basename(fields[0])
		annotation_label = '%s_%d_' % (fields[0], counter)
		final_label = annotation_label+'_'.join(os.path.basename(baseName).split('_')[1:])
		filename = os.path.join(savePath, '%s' % final_label)
	else:
